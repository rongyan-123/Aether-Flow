...
import { ICharacterStats, IInventoryItem, IRelationships, IntentType } from '@/types'
import { ragRetrieverNode, plotDirectorNode, ruleEngineNode, dbPersistNode, setLLMConfig } from './nodes'
...
export const GameStateAnnotation = Annotation.Root({
  ...
  stats: Annotation<ICharacterStats>({
    reducer: (x: ICharacterStats, y: Partial<ICharacterStats>) => ({ ...x, ...y }),
  }),
